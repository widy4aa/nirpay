...
export default function TopUpPage() {
  ...
  const { transactions, total, isLoading, error, mutate } = usePendingTransactions('TOPUP');
  ...
  const handleApprove = async (txId: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/wallet/topup/${txId}/approve`, {
        method: 'POST',
        headers: {
  ...
  const handleReject = async (txId: string) => {
    ...
    setProcessingId(txId);
    try {
      const res = await fetch(`${API_URL}/api/wallet/topup/${txId}/reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${session?.user?.accessToken}`,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Kelola permintaan top up dari pengguna</p>
        </div>
