...
class _RegisterStep5PageState extends ConsumerState<RegisterStep5Page> {
  ...
  void _onNext() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Mengupload foto KTP...'),
    ...
    try {
      ...
      final ktpUrl = await uploadService.uploadKtp(_imagePath!);

      ref.read(registrationFormProvider.notifier).update((state) {
        return state.copyWith(ktpPhotoUrl: ktpUrl);
      });

      if (mounted) {
        ScaffoldMessenger.of(context).clearSnackBars();
        context.pushNamed(AppRouteNames.registerStep6);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildHeader() {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 12),
        Text(
          'Ambil foto KTP Anda dengan jelas. Pastikan\nteks pada KTP terbaca dengan baik.',
          style: TextStyle(
            fontSize: 14,
